...
###########################################dxy###########################################
dxy_stat <- file_table  %>%  ggplot(aes(population,avg_dxy,fill=selscan_heterogeneity))+
...
##########################################Fst############################################
Fst_stat <- file_table  %>%  ggplot(aes(population,avg_wc_fst,fill=selscan_heterogeneity))+
	geom_boxplot(outlier.shape = NA,width = 0.5)+   
...
###########################################pi#########################################
pop_pi_stat <- file_table  %>%  ggplot(aes(population,avg_pi.pop,fill=selscan_heterogeneity))+
  geom_boxplot(outlier.shape = NA,width = 0.5)+  
  coord_cartesian(ylim=c(0, 0.03))+
  theme_classic() +
  stat_compare_means(aes(group = selscan_heterogeneity), method ="wilcox.test",label = "p.signif",label.y = 0.025)+
